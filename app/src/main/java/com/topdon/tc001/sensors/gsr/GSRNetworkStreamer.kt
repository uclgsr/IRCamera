...
class GSRNetworkStreamer(
    ...
    suspend fun initialize(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                networkClient = EnhancedNetworkClient(context).apply {
    ...
    suspend fun startStreaming(): Boolean {
        ...
        return try {
            ...
            qualityReportingJob = streamingScope.launch {
    ...
    private suspend fun sendStreamEndNotification() {
        try {
            val endNotification = JSONObject().apply {
                put("type", "stream_end")
                put("stream_type", GSR_STREAM_TYPE)
                put("session_id", sessionId)
