package com.topdon.tc001.ui_components
...
class ComprehensiveSensorStatusWidget @JvmOverloads constructor(
    ...
    private fun setupUI() {
        ...
        recordingIndicator = ImageView(context).apply {
            layoutParams = LayoutParams(24, 24).apply {
                marginEnd = 8
            }
            setBackgroundColor(COLOR_DISCONNECTED)
        }
        recordingSection.addView(recordingIndicator)
        ...
        sensorsContainer = LinearLayout(context).apply {
            orientation = VERTICAL
            setPadding(8, 0, 8, 0)
        }
    ...
    fun updateRecordingStatus(recording: Boolean, sessionId: String? = null) {
        ...
        if (recording) {
            ...
            recordingTimer.text = "⏱️ Ready to Record"
            stopRecordingTimer()
        }
    ...
    private fun startRecordingTimer() {
        ...
        timerUpdateJob = (context as? LifecycleOwner)?.lifecycleScope?.launch {
            while (isRecording) {
