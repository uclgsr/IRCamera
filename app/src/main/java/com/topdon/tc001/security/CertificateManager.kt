...
class CertificateManager(
    ...
    fun initialize(): Boolean {
        return try {
            ...
            logger.log(StructuredLogger.LogLevel.INFO, TAG, "cert_manager_initialized", mapOf(
    ...
    fun validateCertificate(
        ...
        return try {
            ...
            logger.log(StructuredLogger.LogLevel.INFO, TAG, "cert_validated", mapOf(
            ...
            logger.log(StructuredLogger.LogLevel.ERROR, TAG, "cert_validation_failed", mapOf(
                "device_id" to deviceId,
                "error" to e.message.orEmpty()
            ))
    ...
    private fun generateDeviceCertificate() {
        try {
            ...
            logger.log(StructuredLogger.LogLevel.INFO, TAG, "device_cert_generated", mapOf(
     ...
     */
    private fun saveDeviceCertificate(certificate: X509Certificate, keyPair: KeyPair) {
        try {
            val certFile = File(certDirectory, DEVICE_CERT_FILE)
            val keyFile = File(certDirectory, DEVICE_KEY_FILE)
    ...
    private fun rotateCertificate() {
        try {
            ...
            logger.log(StructuredLogger.LogLevel.INFO, TAG, "cert_rotated", mapOf(
            ...
            logger.log(StructuredLogger.LogLevel.ERROR, TAG, "cert_rotation_failed", mapOf(
                "error" to e.message.orEmpty()
            ))
        }
    }
     ...
     */
    fun encryptData(data: ByteArray, certificate: X509Certificate): ByteArray? {
        return try {
