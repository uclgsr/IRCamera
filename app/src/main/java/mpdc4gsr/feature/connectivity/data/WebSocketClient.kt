...
import mpdc4gsr.app.runtime.session.SessionWorkflowManager
...
class WebSocketClient(private val context: Context) {
    ...
    private var sessionManager: SessionWorkflowManager? = null
    ...
    private fun stopServerDiscovery() {
        try {
            ...
            mpdc4gsr.core.common.AppLogger.e("WebSocketClient", "Unexpected Exception in WebSocketClient catch block", e)
        }
    }
    ...
    private fun initializePhase2Services() {
        ...
        sessionManager =
            SessionWorkflowManager(context, logger).apply {
                start(
    ...
    private fun performCrossDeviceSync(devices: List<SessionWorkflowManager.DeviceInfo>) {
        scope.launch {
            try {
                val syncMessage =
                    JSONObject().apply {
                ...
                devices.forEach { device ->
                    sessionManager?.updateDeviceHeartbeat(
                        device.deviceId,
                        0L,
                        SessionWorkflowManager.ConnectionQuality.GOOD,
                    )
                }
            } catch (e: Exception) {
            }
