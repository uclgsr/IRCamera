...
import mpdc4gsr.feature.connectivity.data.RecordingConstants.RGB_STORAGE_MB_PER_MIN
import mpdc4gsr.feature.connectivity.data.RecordingConstants.SHIMMER_STORAGE_MB_PER_MIN
import mpdc4gsr.feature.connectivity.data.RecordingConstants.THERMAL_STORAGE_MB_PER_MIN
...
class RecordingController(
    ...
    companion object {
        ...
        typealias SessionManifest = mpdc4gsr.feature.connectivity.data.SessionManifest
    ...
    private val networkServerRef = AtomicReference<NetworkServer?>()
    ...
    fun registerSensor(sensorName: String, sensorRecorder: SensorRecorder) {
        ...
        if (sensorRecorder is ThermalCameraRecorder) {
            networkServerRef.get()?.let { server ->
                sensorRecorder.enableNetworkStreaming(server)
            }
        }
    }
    ...
    fun unregisterSensor(sensorName: String) {
        sensorRecorders.remove(sensorName)?.let { sensor ->
            if (sensor is ThermalCameraRecorder) {
                sensor.disableNetworkStreaming()
            }
        }
    }
    ...
    fun attachNetworkServer(server: NetworkServer) {
        ...
        (sensorRecorders["Thermal"] as? ThermalCameraRecorder)?.enableNetworkStreaming(server)
    }

    fun detachNetworkServer(server: NetworkServer? = null) {
        val current = networkServerRef.get()
        if (current != null && (server == null || current === server || current == server)) {
            if (networkServerRef.compareAndSet(current, null)) {
                (sensorRecorders["Thermal"] as? ThermalCameraRecorder)?.disableNetworkStreaming()
            }
        }
    }

    fun getNetworkServer(): NetworkServer? = networkServerRef.get()
    ...
    controllerScope.launch {
        try {
            ...
            mpdc4gsr.core.common.AppLogger.e(
                "RecordingController",
                "Unexpected Exception in RecordingController catch block",
                e
            )
        }
    }
...
fun generateSessionManifest(): SessionManifest {
    ...
    val convertedEvents = sessionEvents.map { event ->
        mpdc4gsr.feature.connectivity.data.SessionEvent(
    ...
    return SessionManifest(
        ...
        triggerSource = convertFromRecordingControllerTriggerSource(lastTriggerSource)
            ?: mpdc4gsr.feature.connectivity.data.TriggerSource.LOCAL_UI,
...
private fun convertFromRecordingControllerTriggerSource(source: RecordingController.TriggerSource?): mpdc4gsr.feature.connectivity.data.TriggerSource? {
    return when (source) {
        RecordingController.TriggerSource.LOCAL_UI -> mpdc4gsr.feature.connectivity.data.TriggerSource.LOCAL_UI
        RecordingController.TriggerSource.LOCAL_NOTIFICATION -> mpdc4gsr.feature.connectivity.data.TriggerSource.LOCAL_NOTIFICATION
        RecordingController.TriggerSource.REMOTE_PC -> mpdc4gsr.feature.connectivity.data.TriggerSource.REMOTE_PC
        RecordingController.TriggerSource.AUTOMATIC -> mpdc4gsr.feature.connectivity.data.TriggerSource.AUTOMATIC
        RecordingController.TriggerSource.CRASH_RECOVERY -> mpdc4gsr.feature.connectivity.data.TriggerSource.CRASH_RECOVERY
        null -> null
    }
}

private fun convertFromRecordingControllerSessionState(state: RecordingController.SessionState): mpdc4gsr.feature.connectivity.data.SessionState {
    return when (state) {
        RecordingController.SessionState.IDLE -> mpdc4gsr.feature.connectivity.data.SessionState.IDLE
        RecordingController.SessionState.STARTING -> mpdc4gsr.feature.connectivity.data.SessionState.STARTING
        RecordingController.SessionState.RECORDING -> mpdc4gsr.feature.connectivity.data.SessionState.RECORDING
        RecordingController.SessionState.STOPPING -> mpdc4gsr.feature.connectivity.data.SessionState.STOPPING
        RecordingController.SessionState.STOPPED_COMPLETED -> mpdc4gsr.feature.connectivity.data.SessionState.STOPPED_COMPLETED
        RecordingController.SessionState.STOPPED_FAILED -> mpdc4gsr.feature.connectivity.data.SessionState.STOPPED_FAILED
        RecordingController.SessionState.STOPPED_INCOMPLETE -> mpdc4gsr.feature.connectivity.data.SessionState.STOPPED_INCOMPLETE
    }
}
}
