...
import mpdc4gsr.feature.main.domain.model.GsrDataState
...
class MainActivityViewModel @Inject constructor(
    ...
    private fun stopRecordingSession() {
        viewModelScope.launch {
            val sessionId = _uiState.value.currentSession?.sessionId ?: run {
    ...
    private fun handleGsrAction(action: GSRAction) {
        when (action) {
            ...
            GSRAction.Disconnect -> viewModelScope.launch {
            ...
            GSRAction.StartStream -> updateGsrSensorState(
                status = SensorStatus.STREAMING,
                message = "Streaming GSR data",
                isRecording = true
            )
            GSRAction.StopStream -> updateGsrSensorState(
                status = SensorStatus.CONNECTED,
                message = "Stream stopped",
                isRecording = false
            )
            is GSRAction.ConfigureDevice -> viewModelScope.launch {
    ...
    private fun handleThermalAction(action: ThermalAction) {
        when (action) {
            ThermalAction.Connect -> updateThermalSensorState(
                status = SensorStatus.CONNECTING,
                message = "Connecting thermal camera..."
            )
            ThermalAction.Disconnect -> updateThermalSensorState(
                status = SensorStatus.DISCONNECTED,
                message = "Thermal camera disconnected"
            )
            ThermalAction.StartPreview -> updateThermalSensorState(
                status = SensorStatus.STREAMING,
                message = "Thermal preview active"
            )
            ThermalAction.StopPreview -> updateThermalSensorState(
                status = SensorStatus.CONNECTED,
    ...
    private fun handleCameraAction(action: CameraAction) {
        when (action) {
            CameraAction.Connect -> updateRgbSensorState(
                status = SensorStatus.CONNECTING,
                message = "Connecting RGB camera..."
            )
            CameraAction.Disconnect -> updateRgbSensorState(
                status = SensorStatus.DISCONNECTED,
                message = "RGB camera disconnected"
            )
            CameraAction.StartPreview -> updateRgbSensorState(
                status = SensorStatus.STREAMING,
                message = "RGB preview active"
            )
            CameraAction.StopPreview -> updateRgbSensorState(
                status = SensorStatus.CONNECTED,
                message = "RGB preview stopped"
            )
            is CameraAction.SetResolution -> emitToast(
    ...
    private fun observeGsrState() {
        collectInViewModelScope {
            launch {
                gsrRepository.getConnectionState()
            ...
            launch {
                gsrRepository.getBatteryLevel()
                    .collect { level ->
                        _uiState.update {
                            it.copy(
                                gsrBatteryLevel = level,
                                gsrDataState = it.gsrDataState.copy(
            ...
            launch {
                gsrRepository.getConnectionQuality()
                    .collect { quality ->
                        _uiState.update {
    ...
    private fun observeNetworkState() {
        collectInViewModelScope {
            launch {
                networkRepository.getConnectionState()
                    .distinctUntilChanged()
                    .collect { state ->
            ...
            launch {
                networkRepository.getConnectedController()
                    .collect { controller ->
    ...
    private fun mapGsrConnectionState(state: GsrConnectionState): SensorStatus = when (state) {
    ...
    private fun emitToast(message: String, isLong: Boolean = false) {
        viewModelScope.launch {
