...
class RecordingService :
    ...
    inner class RecordingServiceBinder : Binder() {
        ...
        fun getServerStatus(): String =
            if (::pcControllerServer.isInitialized) {
                pcControllerServer.describeStatus()
            } else {
                "Stopped"
            }
        ...
        fun getConnectedClients(): List<String> =
            if (::pcControllerServer.isInitialized) {
                pcControllerServer.connectedClients()
            } else {
                emptyList()
            }
        ...
        fun getNetworkManager(): NetworkManager? = if (::networkManager.isInitialized) networkManager else null
    }
    ...
    override fun onCreate() {
        ...
        recordingController.attachNetworkServer(networkServer)
        ...
        pcControllerServer =
            PcControllerServer(
                ...
                crashSafeSupervisor = crashSafeSupervisor,
                deviceId = deviceId,
                callbacks =
                    object : PcControllerServer.Callbacks {
                        override suspend fun onSessionStartRequested(sessionDirectory: File) {
                            withContext(Dispatchers.Main) {
                                startRecordingSession(sessionDirectory.absolutePath)
                            }
                        }

                        override suspend fun onSessionStopRequested() {
                            withContext(Dispatchers.Main) {
                                stopRecordingSession()
                            }
                        }

                        override suspend fun onSyncFlashRequested(durationMs: Int) {
                            withContext(Dispatchers.Main) {
                                performSyncFlash(durationMs)
                            }
                        }

                        override suspend fun provideRecordingState(): PcControllerServer.RecordingState =
                            PcControllerServer.RecordingState(
                                isRecording = recordingController.isRecording,
                                currentSessionDirectory = currentSessionDirectory,
                                sensorsInitialized = isInitialized,
                            )
                    },
                listener =
                    object : PcControllerServer.Listener {
                        override fun onServerStarted(port: Int) {
                            val message = "Server listening for PC connections on port $port"
                        ...
                        override fun onClientCountChanged(count: Int) {
                            updateNotification("Connected PCs: $count")
                        }

                        override fun onServerFailure(throwable: Throwable) {
                            updateNotification("Server error: ${throwable.message ?: "Unknown"}")
                        }
                    },
                config =
                    PcControllerServer.Config(
                        basePort = SERVER_PORT,
                        serviceName = SERVICE_NAME,
                        serviceType = SERVICE_TYPE,
                        enableMdns = FeatureFlags.MDNS_ENABLE,
                    ),
            )
    ...
    override fun onDestroy() {
        ...
        try {
            ...
            if (::recordingController.isInitialized) {
                recordingController.detachNetworkServer()
                launch {
    ...
    private fun startRecordingSession(sessionDirectory: String) {
        launch {
            val success =
                startRecordingSessionWithTrigger(
                    sessionDirectory,
    ...
    private suspend fun stopRecordingSessionWithTrigger(triggerSource: TriggerSource): Boolean =
        try {
            ...
            if (success) {
                ...
                val completedNotificationText =
                    when (triggerSource) {
                        TriggerSource.REMOTE_PC ->
                            "Recording completed via PC (${
                                String.format(
                                    "%.1f",
                                    sessionDuration,
                                )
                        ...
                        TriggerSource.LOCAL_NOTIFICATION ->
                            "Recording completed via notification (${
                                String.format(
                                    "%.1f",
                                    sessionDuration,
                                )
            ...
            val serverRunning =
                ::pcControllerServer.isInitialized && pcControllerServer.isRunning()
            if (!serverRunning) {
                stopSelf()
            }
            success
    ...
    private fun writeRecoveredSessionManifest(
        ...
        try {
            ...
            val sensorActivitySummary =
                sensors.associateWith { sensorName ->
    ...
    private fun broadcastRecoveryEvent(
        ...
        val intent =
            Intent(ACTION_CRASH_RECOVERY_COMPLETED).apply {
            ...
            if (isServiceForeground()) {
                ...
                val notification =
                    when {
                        ...
                        serverActive -> createServerNotification(contentText)
                        else -> null
                    }
    ...
    fun getCurrentSession(): SessionInfo? =
        currentSessionDirectory?.let { directory ->
            SessionInfo(
    ...
    private fun startServerSocket() {
        if (::pcControllerServer.isInitialized) {
            pcControllerServer.start()
        }
    }

    private fun stopServerSocket() {
        if (::pcControllerServer.isInitialized) {
            pcControllerServer.stop()
        }
    }
    ...
    private fun setupNetworkServer() {
        ...
        protocolHandler.setCommandHandler(
            object : ProtocolHandler.CommandHandler {
                ...
                override suspend fun onSyncRequest(pcTimestamp: Long): ProtocolHandler.SyncResult =
                    try {
                        val timeManager =
                            mpdc4gsr.core.data.utils.TimeManager
                                .getInstance(this@RecordingService)
                        val phoneTimestamp =
    ...
    private fun disconnectFromPC() {
        launch {
            try {
                ...
                recordingController.detachNetworkServer(networkServer)
                isConnectedToPC = false
    ...
    private suspend fun handleProtocolMessage(message: mpdc4gsr.feature.connectivity.data.Protocol.ProtocolMessage) {
        try {
            ...
            val errorResponse =
                mpdc4gsr.feature.connectivity.data.Protocol.createErrorMessage(
                    message.type,
                    mpdc4gsr.feature.connectivity.data.Protocol.ERR_FAIL,
    ...
    private fun isServiceForeground(): Boolean {
        val serverActive =
            ::pcControllerServer.isInitialized && pcControllerServer.isRunning()
        return recordingController.isRecording || serverActive
    }
