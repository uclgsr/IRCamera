...
class TimeSyncManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val CSV_HEADER = "sync_index,timestamp_iso,phone_timestamp_t2,pc_send_time_t1,pc_recv_time_t3,offset_ms,rtt_ms,session_relative_time_ms,sync_quality,retry_count"
    ...
    fun updateSyncConfiguration(config: SyncConfiguration) {
        syncConfig = config
        Log.i(TAG, "Sync configuration updated: periodicInterval=${config.periodicSyncIntervalMs}ms, " +
    ...
    private fun validateTimestamp(timestamp: Long, context: String): Boolean {
        ...
        return when {
            timeDiff > syncConfig.maxFutureTimestampMs -> {
                Log.w(TAG, "$context timestamp too far in future: ${timeDiff}ms")
                false
            }
            timeDiff < -syncConfig.maxTimestampDriftMs -> {
                Log.w(TAG, "$context timestamp too far in past: ${timeDiff}ms")
                false
            }
            else -> true
        }
        ...
        return withContext(Dispatchers.IO) {
            ...
            repeat(syncConfig.maxSyncRetries) { attempt ->
                try {
                    ...
                    retryCount = attempt + 1
                    Log.w(TAG, "Sync response attempt ${attempt + 1} failed", e)
            ...
            Log.e(TAG, "All sync response attempts failed after $retryCount retries")
    ...
    suspend fun completeSyncCalculation(
        ...
        repeat(syncConfig.maxSyncRetries) { attempt ->
            try {
                ...
                if (!validateTimestamp(t1, "PC send time") ||
                    ...
                    Log.w(TAG, "Invalid timestamps in sync calculation, skipping")
                    return
                }
                ...
                if (logged) {
                    ...
                    retryCount = attempt + 1
                    Log.w(TAG, "Failed to log sync result, attempt ${attempt + 1}")
                    if (attempt < syncConfig.maxSyncRetries - 1) {
                        delay(syncConfig.retryDelayMs)
                    }
                }
                ...
                retryCount = attempt + 1
                Log.w(TAG, "Sync calculation attempt ${attempt + 1} failed", e)
                if (attempt < syncConfig.maxSyncRetries - 1) {
                    delay(syncConfig.retryDelayMs)
                }
            }
        ...
        Log.e(TAG, "Failed to complete sync calculation after $retryCount retries")
    }
        ...
        try {
            ...
            FileWriter(logFile, true).use { writer ->
                ...
                if (syncConfig.enableCsvLogging) {
                    val csvEntry = "${result.syncIndex},$timestamp,${result.t2},${result.t1},${result.t3},${result.offsetMs},${result.rttMs},$sessionRelativeTime,${result.quality},${result.retryCount}"
                    writer.write("$csvEntry\n")
                }
            }
            
            Log.d(TAG, "Logged sync result: index=${result.syncIndex}, offset=${result.offsetMs}ms, rtt=${result.rttMs}ms, quality=${result.quality}")
