...
class TemperatureHikView : TemperatureBaseView {
    /**
// 要drawing的temperature信息
     */
    @Volatile
    private var tempInfo = TempInfo()

    /**
// calculationtemperature的工具类.
     */
    private var libIRTemp = LibIRTemp()

    /**
// calculationtemperature的线程.
     */
    private var calculateThread: CalculateThread? = null

    /**
// thermal imaging画面逆时针rotation角度，取值 0、90、180、270，默认 270
     */
    @Volatile
...
// temperature变更事件监听，temperature单位均为 **摄氏度**
    ...
    /**
// 趋势图直线对应的temperaturedata变更监听，单位摄氏度.
     */
...
// temperature measurement结果回调，单位均为摄氏度.
    ...
    /**
// 当尚未经过 onMeasure 调用添加点时，save要添加的以 temperature scale寸 为坐标的点，在 onMeasure 阶段添加。
     */
    private var wantAddPoint: Point? = null

    /**
// 当尚未经过 onMeasure 调用添加点时，save要添加的以 temperature scale寸 为坐标的线，在 onMeasure 阶段添加。
     */
    private var wantAddLine: Line? = null

    /**
// 当尚未经过 onMeasure 调用添加点时，save要添加的以 temperature scale寸 为坐标的面，在 onMeasure 阶段添加。
     */
    private var wantAddRect: Rect? = null

    /**
// 添加一个以 temperature scale寸 为坐标的点
     */
    ...
    /**
// 添加一个以 temperature scale寸 为坐标的线
     */
    ...
    /**
// 添加一个以 temperature scale寸 为坐标的面
     */
    ...
    /**
// 用于temperature及画面rotationparameter的尺寸.
     */
...
// 未rotation前的temperaturearray.
    ...
    /**
// rotation后的temperaturearray，趋势图要用，而 [libIRTemp] 又没提供方法读取里面的data，只好再搞一份
     */
...
// 以 旧坐标及新 scale 去calculationtemperature坐标的话，就有可能超出temperature坐标范围从而抛出异常，这里捕获
    ...
    /**
// 点线面full image等temperaturecalculation结果信息封装，坐标采用 View 坐标，单位均为摄氏度
     */
/**
