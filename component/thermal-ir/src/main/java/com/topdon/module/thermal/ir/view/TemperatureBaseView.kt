...
/**
// TC007、2D 编辑 点线面temperature图层公共逻辑封装.
 *
...
// temperature值文字大小，单位 px.
    ...
    /**
// temperature值文字、点线面名称文字 color value.
     */
    var textColor: Int
    ...
    /**
// 由于 Touch 事件导致的点添加、移除、变更事件监听，坐标为通过 [setImageSize] set的坐标系
     */
    var onPointListener: ((pointList: List<Point>) -> Unit)? = null

    /**
// 由于 Touch 事件导致的线添加、移除、变更事件监听，坐标为通过 [setImageSize] set的坐标系
     */
    var onLineListener: ((lineList: List<Point>) -> Unit)? = null

    /**
// 由于 Touch 事件导致的面添加、移除、变更事件监听，坐标为通过 [setImageSize] set的坐标系
     */
...
// @param point 以 View 尺寸为坐标系的点
    ...
    /**
// 以 View 尺寸为坐标系，连接 (startX, startY)、(stopX, stopY) 两点drawing一条线段.
     */
...
// 故而这里需要把当前的坐标，尽量贴近temperature坐标的整数倍，否则会出现实心圆偏离直线太远的情况
    ...
    /**
// 以 View 尺寸为坐标系，按指定范围drawing一个矩形.
     */
...
// @param isMax true-最high temperature红色 false-最low temperature蓝色
    ...
    /**
// 以 View 尺寸为坐标系，指定的 (x,y) 坐标为实心圆圆心，以该实心圆为基准drawing指定文字。
...
// @param x 实心圆圆心的 View 尺寸坐标
    ...
    /**
// 以 View 尺寸为坐标系，以指定线段为基准drawing趋势图的 "A"、"B" 文字。
     *
// 注意，不对 x、y 进行processing，传进来是哪就在哪drawing。
     */
    ...
    /**
// 以 View 尺寸为坐标系，指定的 (x,y) 坐标为实心圆圆心，以该实心圆为基准drawing指定点名称文字。
...
// 由于十字与实心点的的drawing是分开的，十字使用当前 View 坐标，而实心点使用temperature(192x256)坐标
// 故而这里需要把当前的坐标，conversion为temperature坐标的整数倍，否则会出现中心对不上的情况
...
// 线移动方式：整体移动、仅变更头、仅变更尾。
    ...
    /**
// 仅整体移动线时，save DOWN 状态下的线初始坐标，用于calculation移动.
     */
...
// 仅角移动模式时，移动的是哪个角.
    ...
    /**
// 移动面时，save DOWN 状态下的面初始坐标，用于calculation移动.
     */
