...
delete操作灵敏度，当 Touch UP 与 Touch Down 坐标偏差在该值range内，视为delete，单位 px.
    ...
    /**
操作mode，point、line、area、full image、Clear.
     */
    enum class Mode {
        POINT,
        LINE,
...
temperature值text、pointlineareanametext color value.
    ...
    /**
由于 Touch Event导致的pointadd、移除、变更EventListener，坐标为通过 [setImageSize] set的坐标系
     */
    var onPointListener: ((pointList: List<Point>) -> Unit)? = null

    /**
由于 Touch Event导致的lineadd、移除、变更EventListener，坐标为通过 [setImageSize] set的坐标系
     */
    var onLineListener: ((lineList: List<Point>) -> Unit)? = null

    /**
由于 Touch Event导致的areaadd、移除、变更EventListener，坐标为通过 [setImageSize] set的坐标系
     */
