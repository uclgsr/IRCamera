...
object IRImageUtil {
    ...
    fun showContrast(
        ...
        try {
            ...
            val lookUpTableData = ByteArray((lookUpTable.total() * lookUpTable.channels()).toInt())            for (i in 0 until lookUpTable.cols()) {
                if (i % 10 == 0) {* 255.0
                        }"
                    )
                }
    ...
    private fun showSharpen(
        ...
        val preGray = Mat()
        val absDst = Mat()        Imgproc.GaussianBlur(srcMat, srcMat, Size(3.0, 3.0), 0.0, 0.0, BORDER_DEFAULT)
        Imgproc.cvtColor(srcMat, preGray, Imgproc.COLOR_BGR2GRAY)        Imgproc.Laplacian(srcMat, dstMat, CV_16S, kernelSize, scale, delta, BORDER_DEFAULT)        Core.convertScaleAbs(dstMat, absDst)        val preMat = Mat()
