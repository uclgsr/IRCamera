...
import com.mpdc4gsr.component.shared.ir.inf.ILiteListener;
import com.mpdc4gsr.component.shared.ir.usbdual.Const;
import com.mpdc4gsr.component.shared.ir.usbdual.camera.BaseDualView;
...
public class TemperatureView extends SurfaceView implements SurfaceHolder.Callback,
                    ...
                    if (regionAndValueBitmap != null) {
                        synchronized (regionLock) {
                            ...
                            if (isShowFull) {
                                ...
                                int x = SharedScreenUtils.correct(fullResult.minTemperaturePixel.x * xScale, getWidth());
                                int y = SharedScreenUtils.correct(fullResult.minTemperaturePixel.y * yScale, getHeight());
                                drawCircle(canvas, x, y, false);
                                drawTempText(canvas, minTem, x, y);
                            }
                            if (isUserLowTemp) {
                                int x = SharedScreenUtils.correctPoint(fullResult.minTemperaturePixel.x * xScale, getWidth());
                                int y = SharedScreenUtils.correctPoint(fullResult.minTemperaturePixel.y * yScale, getHeight());
                            ...
                            if (isShowFull) {
                                ...
                                int x = SharedScreenUtils.correct(fullResult.maxTemperaturePixel.x * xScale, getWidth());
                                int y = SharedScreenUtils.correct(fullResult.maxTemperaturePixel.y * yScale, getHeight());
                                drawCircle(canvas, x, y, true);
                                drawTempText(canvas, maxTem, x, y);
                            }
                            if (isUserHighTemp) {
                                int x = SharedScreenUtils.correctPoint(fullResult.maxTemperaturePixel.x * xScale, getWidth());
                                int y = SharedScreenUtils.correctPoint(fullResult.maxTemperaturePixel.y * yScale, getHeight());
                                drawPoint(canvas, x, y);
                                drawCircle(canvas, x, y, true);
                            }
                            ...
                            if (trendLine != null) {
                                ...
                                if (maxX < temperatureWidth && minX > 0 && maxY < temperatureHeight && minY > 0) {
                                    ...
                                    if (onTrendChangeListener != null) {
                                        List<Float> tempList = SharedTemperatureUtils.INSTANCE.getLineTemperatures(new Point(startX, startY), new Point(endX, endY), tempArray, temperatureWidth, temperatureHeight);
    ...
    public void addScalePoint(Point point) {
        ...
        int viewX = SharedScreenUtils.correctPoint(point.x * sx, getMeasuredWidth());
        int viewY = SharedScreenUtils.correctPoint(point.y * sy, getMeasuredHeight());
    ...
    public void addScaleLine(Line l) {
        ...
        line.start.x = SharedScreenUtils.correct(l.start.x * sx, getMeasuredWidth());
        line.start.y = SharedScreenUtils.correct(l.start.y * sy, getMeasuredHeight());
        line.end.x = SharedScreenUtils.correct(l.end.x * sx, getMeasuredWidth());
        line.end.y = SharedScreenUtils.correct(l.end.y * sy, getMeasuredHeight());
        if (pointList.size() == POINT_MAX_COUNT) {
            pointList.remove(0);
        }
        lineList.add(line);
    ...
    private boolean handleTouchPoint(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                downX = SharedScreenUtils.correctPoint(event.getX(), getWidth());
                downY = SharedScreenUtils.correctPoint(event.getY(), getHeight());
            ...
            case MotionEvent.ACTION_MOVE: {
                int x = SharedScreenUtils.correctPoint(event.getX(), getWidth());
                int y = SharedScreenUtils.correctPoint(event.getY(), getHeight());
                Canvas surfaceViewCanvas = getHolder().lockCanvas();
                surfaceViewCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
                surfaceViewCanvas.drawBitmap(regionBitmap, new Rect(0, 0, viewWidth, viewHeight), new Rect(0, 0, viewWidth, viewHeight), null);
    ...
    private boolean handleTouchLine(MotionEvent event, boolean isTrend) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                downX = SharedScreenUtils.correct(event.getX(), getWidth());
                downY = SharedScreenUtils.correct(event.getY(), getHeight());
            ...
            case MotionEvent.ACTION_MOVE: {
                int x = SharedScreenUtils.correct(event.getX(), getWidth());
                int y = SharedScreenUtils.correct(event.getY(), getHeight());
                if (isAddAction) {
                    Canvas surfaceViewCanvas = getHolder().lockCanvas();
                    surfaceViewCanvas.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR);
                    surfaceViewCanvas.drawBitmap(regionBitmap, new Rect(0, 0, viewWidth, viewHeight), new Rect(0, 0, viewWidth, viewHeight), null);
                    ...
                    switch (lineMoveType) {
                        case ALL:
                            Rect rect = SharedScreenUtils.getRect(getWidth(), getHeight());
    ...
    private void drawDot(Canvas canvas, Point point, boolean isMax) {
        ...
        int x = SharedScreenUtils.correct(point.x * xScale, getWidth());
        int y = SharedScreenUtils.correct(point.y * yScale, getHeight());
        tempPaint.setColor(isMax ? Color.RED : Color.BLUE);
        canvas.drawCircle(x, y, 10f, tempPaint);
    }
