...
object SharedDirectoryUtils {
    ...
    fun getCacheDirectory(context: Context): File = File(getAppRootDirectory(context), CACHE_DIR).apply { mkdirs() }

    fun getLogsDirectory(context: Context): File = File(getAppRootDirectory(context), LOGS_DIR).apply { mkdirs() }

    fun getConfigDirectory(context: Context): File = File(getAppRootDirectory(context), CONFIG_DIR).apply { mkdirs() }

    fun getTempDirectory(context: Context): File = File(getAppRootDirectory(context), TEMP_DIR).apply { mkdirs() }

    fun getFeatureDirectory(
        context: Context,
        featureName: String,
    ): File =
        File(getAppRootDirectory(context), featureName.lowercase()).apply {
            mkdirs()
        }
    ...
    fun getFormattedDirectorySize(directory: File): String {
        ...
        return SharedDataUtils.formatDataSize(size)
    }
