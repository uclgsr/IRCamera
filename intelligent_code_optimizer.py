...
class IntelligentCodeOptimizer:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_file_path ON file_metrics(file_path)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_timestamp ON quality_trends(timestamp)"
            )
        ...
        try:
            if file_type == "kotlin" and self._command_exists("ktlint"):
                ...
                if checkstyle_config.exists():
                    result = subprocess.run(
                        ["checkstyle", "-c", str(checkstyle_config), str(file_path)],
                        capture_output=True,
                        text=True,
                        timeout=30,
                    )
    ...
    def analyze_project(self) -> Dict[str, List[FileMetrics]]:
        ...
        for file_type, files in files_by_type.items():
            ...
            if (
                self.config["optimization"]["enable_parallel_processing"]
                and len(files) > 10
            ):
    ...
    def store_metrics(self, results: Dict[str, List[FileMetrics]]):
        ...
        with sqlite3.connect(self.db_path) as conn:
                ...
                for metrics in metrics_list:
                    ...
                    """,
                        (
                            ...
                            metrics.last_modified.isoformat(),
                            metrics.processing_time,
                            metrics.hash_signature,
                            timestamp,
                        ),
                    )
            ...
            total_violations = sum(
                len(metrics_list) for metrics_list in results.values()
            )
            ...
            violation_score = max(
                0,
                100
                - (self.total_violations_fixed / max(total_files, 1))
                * 100
    ...
    def _generate_insights(self, trends: List[Tuple]) -> List[str]:
        ...
        if quality_change > 5:
            insights.append(
                "🎉 Significant quality improvement detected! Keep up the excellent work."
            )
        elif quality_change < -5:
            insights.append(
                "⚠️ Quality regression detected. Consider reviewing recent changes."
            )
        else:
            insights.append(
                "📊 Quality remains stable. Consider implementing new optimization strategies."
            )
        ...
        if complexity_change > 2:
            insights.append(
                "🔍 Code complexity is increasing. Consider refactoring complex functions."
            )
        elif complexity_change < -2:
            insights.append(
                "✨ Code complexity is decreasing! Excellent refactoring work."
            )
        ...
        if speed_change > 0.5:
            insights.append(
                "⚡ Analysis performance improved! Optimization efforts are paying off."
            )
        elif speed_change < -0.5:
            insights.append(
                "🐌 Analysis performance decreased. Consider optimizing the analysis pipeline."
            )

        return insights

    def generate_html_report(
        self, results: Dict[str, List[FileMetrics]], trend_data: Dict
    ):
...
"""
        ...
        for file_type, metrics_list in results.items():
            if metrics_list:
                avg_complexity = sum(m.complexity_score for m in metrics_list) / len(
                    metrics_list
                )
...
"""
    ...
    def run_analysis(self):
        ...
        print(
            f"📊 Configuration: {self.max_workers} workers, {len(self.config['file_types'])} file types"
        )
        ...
        try:
            ...
            summary = {
                ...
                "quality_score": trend_data.get("latest_analysis", {}).get(
                    "quality_score", 0
                ),
                "file_type_summary": {
                    file_type: {
                        ...
                        "total_violations": sum(
                            m.violations_count for m in metrics_list
                        ),
                    }
                    for file_type, metrics_list in results.items()
                },
