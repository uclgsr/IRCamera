...
This consolidation effort focused on removing duplicate code, unused files, and unnecessary legacy scripts from the IRCamera repository to reduce clutter and improve maintainability.
...
**Reason**: These one-time migration scripts had completed their purpose. All architectural transformations (legacy to Clean Architecture, XML to Compose) are complete. Keeping them only added clutter.
...
- `BaseIRPlusFragment.kt` - Changed from `ScreenUtils` to `UnifiedScreenUtils`
  - Updated import: `com.mpdc4gsr.libunified.ir.utils.ScreenUtils` → `com.mpdc4gsr.libunified.app.utils.UnifiedScreenUtils`
...
This aggressive consolidation successfully removed over 2,200 lines of duplicate and unused code while maintaining full functionality. All changes verified through compilation and testing.
