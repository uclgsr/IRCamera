...
interface ThermalHourDao {
    ...
    @Query("SELECT COALESCE(MAX(thermal_max), 0.0) FROM thermal_hour WHERE user_id = :userId AND create_time >= :startTime AND create_time <= :endTime")
    fun queryByTimeMax(userId: String, startTime: Long, endTime: Long): Float

    @Query("SELECT COALESCE(MIN(thermal_min), 0.0) FROM thermal_hour WHERE user_id = :userId AND create_time >= :startTime AND create_time <= :endTime")
