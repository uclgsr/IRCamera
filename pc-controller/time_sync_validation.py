...
class TimeSyncValidator:
    ...
    async def test_ntp_protocol_accuracy(self) -> bool:
        ...
        for scenario_name, base_delay, jitter in test_scenarios:
            print(
                f"  📡 Testing {scenario_name} (delay: {base_delay*1000:.1f}ms ± {jitter*1000:.1f}ms)"
            )
            ...
            print(
                f"    {'✅' if meets_requirement else '❌'} Sub-5ms requirement: {meets_requirement}"
            )
    ...
    async def test_flash_sync_validation(self) -> bool:
        ...
        for i, device in enumerate(devices):
            ...
            flash_timestamp = device_flash_time - int(
                processing_delay * 0.5 * 1_000_000_000
            )
        ...
        for response in device_responses:
            ...
            print(
                f"    {response['device']}: alignment_error={alignment_error_ms:.3f}ms"
            )
        ...
        print(
            f"  {'✅' if meets_requirement else '❌'} Flash sync alignment < 5ms: {meets_requirement}"
        )
    ...
    async def test_multi_device_coordination(self) -> bool:
        ...
        devices = [
            {
                "id": "android_001",
                "name": "Phone-Primary",
                "sensors": ["thermal", "visual", "gsr"],
            },
            {
                "id": "android_002",
                "name": "Tablet-Secondary",
                "sensors": ["visual", "audio"],
            },
        ...
        for device in devices:
            ...
            network_delay = 0.002 + (
                len(device["sensors"]) * 0.001
            )  # More sensors = longer startup
            await asyncio.sleep(network_delay)
            ...
            print(
                f"    {device['name']}: started after {network_delay*1000:.2f}ms ({len(device['sensors'])} sensors)"
            )
        ...
        print(
            f"  {'✅' if meets_requirement else '❌'} Coordination window < 50ms: {meets_requirement}"
        )
    ...
    async def test_clock_offset_calculation(self) -> bool:
        ...
        print(
            f"  {'✅' if meets_requirement else '❌'} Clock offset accuracy < 1ms: {meets_requirement}"
        )
        ...
        return {
            ...
            "summary": {
                ...
                "passed_scenarios": sum(
                    1 for r in self.results if r["meets_requirement"]
                ),
                "average_error_ms": (
                    statistics.mean([r["max_error_ms"] for r in self.results])
                    if self.results
                    else 0
                ),
                "worst_case_error_ms": (
                    max([r["max_error_ms"] for r in self.results])
                    if self.results
                    else 0
                ),
            },
