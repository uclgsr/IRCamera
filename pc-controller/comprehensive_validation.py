...
"""
        ...
        if connection_tests:
            avg_connection_time = sum(t['duration'] for t in connection_tests) / len(connection_tests)
            report += f"- **Average Connection Time**: {avg_connection_time:.3f}s\n"
            
        response_tests = [t for t in self.tests if 'ping' in t['name'].lower()]
        if response_tests:
            avg_response_time = sum(t['duration'] for t in response_tests) / len(response_tests)
        ...
        if passed_tests == total_tests:
            report += "✅ **All Tests Passed**: Implementation is ready for production use.\n"
...
class ComprehensiveValidator:
    ...
    def test_recording_control(self) -> bool:
        ...
        self.report.add_test(
            ...
            success, 
            duration,
            "Recording stop command sent successfully" if success else f"Stop command failed: {response}"
        )
    ...
    def test_sync_markers(self) -> bool:
        ...
        sync_message = {
            ...
            "timestamp_ns": int(time.time() * 1_000_000_000),
            "metadata": {
                "test_id": "comprehensive_validation",
                "marker_sequence": 1
            }
        }
        ...
        self.report.add_test(
            ...
            success, 
            duration,
            "Sync marker sent successfully" if success else f"Sync marker failed: {response}"
        )
    ...
    def test_status_request(self) -> bool:
        ...
        if success:
            try:
                response_data = json.loads(response)
                has_status = "session_stats" in response_data or "sensor_status" in response_data
                self.report.add_test(
                    "Status Request", 
                    has_status, 
                    duration,
                    f"Status response received with valid data" if has_status else "Status response missing expected fields"
                )
                return has_status
            except json.JSONDecodeError:
                self.report.add_test(
            ...
            self.report.add_test(
                ...
                False, 
                duration,
                f"Status request failed: {response}"
            )
            return False
...
def main():
    parser = argparse.ArgumentParser(description='Comprehensive PC-to-Phone communication validation')
