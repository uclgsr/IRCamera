...
def demo_camera_integration():
    ...
    try:
        ...
        def preview_callback(device_id, frame, camera_type):
            ...
            print(
                f"📷 {camera_type.upper()} frame from {device_id}: {frame.shape if hasattr(frame, 'shape') else 'N/A'}")
            ...
            for device_id in devices:
                ...
                if NATIVE_BACKEND_AVAILABLE:
                    ...
                    if total_samples % 50 == 0:
                        print(
                            f"📊 Processed {total_samples} samples ({total_samples / (time.time() - start_time):.1f} samples/sec)")
