...
class Phase4SecurityValidator:
    ...
    async def run_all_tests(self):
        ...
        tests = [
            ("Enhanced Security Manager Initialization", self.test_security_manager_init),
    ...
    async def test_security_manager_init(self):
        ...
        try:
            ...
            print(f"     • Security manager initialized: {has_auth_manager and has_security_monitor}")
    ...
    async def test_multi_tier_auth(self):
        ...
        try:
            ...
            success1, context1, reason1 = await self.security_manager.authenticate_device(
                "test_device_1", 
                1,  # AuthLevel.BASIC
                {"username": "admin", "password": "admin", "device_type": "ANDROID_PHONE"}
            )
            
            # Test certificate authentication (will likely fail without real certificates)
            success2, context2, reason2 = await self.security_manager.authenticate_device(
                "test_device_2",
                2,  # AuthLevel.CERTIFICATE  
                {"certificate": b"fake_cert", "signature": b"fake_sig", "challenge": "test", "device_type": "ANDROID_PHONE"}
            )
            
            # Test token authentication
            success3, context3, reason3 = await self.security_manager.authenticate_device(
                "test_device_3",
                3,  # AuthLevel.TOKEN
                {"token": "test_token", "timestamp": time.time(), "hmac": "test_hmac", "device_type": "ANDROID_PHONE"}
            )
    ...
    async def test_rbac_system(self):
        ...
        try:
            ...
            admin_role = DeviceRole.ADMINISTRATOR if hasattr(DeviceRole, 'ADMINISTRATOR') else None
            if admin_role:
                has_all_perms = admin_role.has_permission("any_permission") if hasattr(admin_role, 'has_permission') else True
    ...
    async def test_permission_enforcement(self):
        ...
        try:
            ...
            if hasattr(self.security_manager, 'check_permission'):
                ...
                has_perm = self.security_manager.check_permission("invalid_token", "test_permission")
                permission_enforcement = not has_perm  # Should be False for invalid token
        ...
        try:
            ...
            try:
                ...
                if 'WebSocketServer' in globals():
                    server = WebSocketServer("localhost", 8444)  # Different port for testing
