...
PYBIND11_MODULE(enhanced_native_backend, m) {
    m.doc() = "Enhanced IRCamera Native Backend - High-performance sensor processing and data handling";
    ...
    py::class_<ircamera::ThermalData>(m, "ThermalData")
        .def(py::init<>())
        ...
        .def("__repr__", [](const ircamera::ThermalData &data) {
            return "<ThermalData frame=" + std::to_string(data.frame_sequence) +
                   " size=" + std::to_string(data.width) + "x" + std::to_string(data.height) +
                   " temp_range=" + std::to_string(data.min_temp_c) + "-" + std::to_string(data.max_temp_c) + "°C>";
        });
    
    // EnhancedShimmer class
    py::class_<ircamera::EnhancedShimmer>(m, "EnhancedShimmer")
    ...
    // DataProcessor class
    py::class_<ircamera::DataProcessor>(m, "DataProcessor")
        .def(py::init<>(), "Create DataProcessor instance")
    ...
    py::class_<ircamera::ThermalProcessor>(m, "ThermalProcessor")
        .def(py::init<>(), "Create ThermalProcessor instance")
    ...
    py::class_<ircamera::MessageProcessor>(m, "MessageProcessor")
        .def(py::init<>(), "Create MessageProcessor instance")
