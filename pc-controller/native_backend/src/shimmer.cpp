...
private:
    ...
    void generate_simulated_data() {
        ...
        if (ppg_enabled_) {
            double ppg_signal = 0.7 + 0.2 * std::sin(phase * 0.05) + 0.05 * std::sin(phase * 0.5);
...
std::vector<bool> detect_motion_artifacts(const std::vector<double>& gsr_data, double threshold) {
