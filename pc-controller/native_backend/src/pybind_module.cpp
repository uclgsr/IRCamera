...
PYBIND11_MODULE(native_backend, m) {
    ...
    py::class_<ircamera::FrameData>(m, "FrameData")
        ...
        .def("get_numpy_array", [](const ircamera::FrameData &frame) {
        ...
        .def("__repr__", [](const ircamera::FrameData &frame) {
    ...
    py::class_<ircamera::CameraConfig>(m, "CameraConfig")
        ...
        .def("__repr__", [](const ircamera::CameraConfig &config) {
    ...
    m.def("get_shimmer_ports", []() {
        ...
        for (int i = 1; i <= 20; i++) {
            ...
            // Simple availability check - try to create handle
...
#else
        // Linux/macOS serial port detection
