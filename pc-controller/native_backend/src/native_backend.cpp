...
PYBIND11_MODULE(ircamera_native_backend, m) {
    m.doc() = "IRCamera Native Backend - High-performance sensor interfaces";

    // Shimmer sensor data structure
    py::class_<ShimmerData>(m, "ShimmerData")
        .def(py::init<>())
        ...
        .def("__repr__", [](const ShimmerData& data) {
            return "<ShimmerData timestamp=" + std::to_string(data.timestamp_ns) +
                   " gsr=" + std::to_string(data.gsr_microsiemens) + "μS valid=" +
                   (data.valid ? "true" : "false") + ">";
        });

    // Shimmer statistics
    py::class_<NativeShimmer::Statistics>(m, "ShimmerStatistics")
        .def(py::init<>())
    ...
    // Webcam frame data structure
    py::class_<WebcamFrame>(m, "WebcamFrame")
        .def(py::init<>())
        ...
        .def("get_data_array", [](const WebcamFrame& frame) {
            // Convert to NumPy array for zero-copy access
            return py::array_t<uint8_t>(
                {frame.height, frame.width, frame.channels},
                {sizeof(uint8_t) * frame.width * frame.channels,
        ...
        .def("__repr__", [](const WebcamFrame& frame) {
            return "<WebcamFrame " + std::to_string(frame.width) + "x" +
                   std::to_string(frame.height) + " frame=" +
                   std::to_string(frame.frame_number) + " valid=" +
                   (frame.valid ? "true" : "false") + ">";
        });

    // Camera properties
    py::class_<NativeWebcam::CameraProperties>(m, "CameraProperties")
        .def(py::init<>())
    ...
    // Camera statistics
    py::class_<NativeWebcam::Statistics>(m, "CameraStatistics")
        .def(py::init<>())
