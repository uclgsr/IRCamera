...
public:
    ...
    std::vector<uint8_t> encode_jpeg(const std::vector<uint8_t>& raw_image, int width, int height, int quality) {
...
public:
    ...
    std::vector<double> raw_to_celsius(const std::vector<uint16_t>& raw_data, double emissivity) {
    ...
    std::vector<uint8_t> apply_colormap(const std::vector<double>& temp_data, const std::string& colormap) {
...
std::map<std::string, std::string> DataProcessor::parse_json_message(const std::string& json_str) {
    return pimpl_->parse_json_message(json_str);
}

std::string DataProcessor::create_json_response(const std::map<std::string, std::string>& fields) {
...
std::vector<uint8_t> DataProcessor::decompress_data(const std::vector<uint8_t>& compressed_data) {
    return pimpl_->decompress_data(compressed_data);
}

std::vector<uint8_t> DataProcessor::encode_jpeg(const std::vector<uint8_t>& raw_image, int width, int height, int quality) {
...
std::vector<std::pair<double, double>> DataProcessor::get_recent_samples(double time_window_seconds) const {
...
std::vector<double> ThermalProcessor::raw_to_celsius(const std::vector<uint16_t>& raw_data, double emissivity) {
    return pimpl_->raw_to_celsius(raw_data, emissivity);
}

std::vector<uint8_t> ThermalProcessor::apply_colormap(const std::vector<double>& temp_data, const std::string& colormap) {
    return pimpl_->apply_colormap(temp_data, colormap);
}

std::vector<double> ThermalProcessor::apply_gaussian_blur(const std::vector<double>& image, int width, int height, double sigma) {
    return pimpl_->apply_gaussian_blur(image, width, height, sigma);
}

std::vector<double> ThermalProcessor::apply_histogram_equalization(const std::vector<double>& image) {
...
std::pair<int, int> ThermalProcessor::find_hotspot_location(const std::vector<double>& temp_data, int width, int height) {
...
std::string MessageProcessor::create_hello_message(const std::string& device_id, const std::vector<std::string>& capabilities) {
...
std::string MessageProcessor::create_sync_response(uint64_t pc_timestamp, uint64_t device_timestamp) {
...
std::string MessageProcessor::create_error_message(const std::string& command, const std::string& error_code, const std::string& message) {
    return pimpl_->create_error_message(command, error_code, message);
}

bool MessageProcessor::parse_message(const std::string& message, std::string& type, std::map<std::string, std::string>& params) {
...
int64_t MessageProcessor::calculate_time_offset(uint64_t t1, uint64_t t2, uint64_t t3, uint64_t t4) {
