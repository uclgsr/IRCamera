...
class DeviceRole(Enum):
    ...
    OPERATOR = (2, {"view_status", "view_sessions", "download_data", "start_recording", "stop_recording"})
    RESEARCHER = (3, {"view_status", "view_sessions", "download_data", "start_recording", "stop_recording", "manage_sessions", "export_data"})
    ADMINISTRATOR = (4, {"*"})
...
class EnhancedAuthenticationManager:
    ...
    async def authenticate(self, device_id: str, auth_level: AuthLevel, 
                          credentials: Dict[str, Any]) -> Tuple[bool, Optional[AuthenticationContext], str]:
        """
        ...
        try:
            ...
            if success:
                ...
                context = await self._create_auth_context(device_id, auth_level, credentials)
                ...
                logger.info(f"Authentication successful for device {device_id} at level {auth_level.name}")
    ...
    async def _authenticate_basic(self, credentials: Dict[str, Any]) -> bool:
        ...
        return username in self.enhanced_credentials and self.enhanced_credentials[username] == password
    
    async def _authenticate_certificate(self, device_id: str, credentials: Dict[str, Any]) -> bool:
    ...
    async def _authenticate_token(self, device_id: str, credentials: Dict[str, Any]) -> bool:
    ...
    async def _create_auth_context(self, device_id: str, auth_level: AuthLevel, 
                                  credentials: Dict[str, Any]) -> AuthenticationContext:
    ...
    def _determine_role(self, device_type: str, auth_level: AuthLevel, 
                       credentials: Dict[str, Any]) -> DeviceRole:
    ...
    async def _handle_auth_failure(self, device_id: str):
        ...
        if len(self.failed_attempts[device_id]) >= MAX_FAILED_ATTEMPTS:
            self.locked_devices[device_id] = current_time + (LOCKOUT_DURATION_MINUTES * 60)
            logger.warning(f"Device {device_id} locked due to {MAX_FAILED_ATTEMPTS} failed attempts")
    ...
    def get_diagnostics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "locked_devices": len([d for d, t in self.locked_devices.items() if time.time() < t]),
...
class EnhancedSecurityMonitor:
    ...
    def report_connection_attempt(self, device_id: str, successful: bool, details: Dict[str, Any] = None):
        ...
        logger.debug(f"Connection attempt reported: {device_id} ({'success' if successful else 'failed'})")
    
    async def _generate_alert(self, alert_type: str, severity: AlertSeverity, 
                            device_id: str, description: str, details: Dict[str, Any]):
        ...
        logger.warning(f"Security alert generated: {alert.alert_type} for device {device_id}")
    ...
    def get_monitoring_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_alerts_count": len([a for a in self.security_alerts if time.time() - a.timestamp < 3600]),
...
class EnhancedSecurityManager:
    ...
    async def authenticate_device(self, device_id: str, auth_level: AuthLevel, 
                                 credentials: Dict[str, Any]) -> Tuple[bool, Optional[AuthenticationContext], str]:
        """Authenticate device with security monitoring"""
        
        # Report connection attempt to monitor
        success, context, reason = await self.auth_manager.authenticate(device_id, auth_level, credentials)
        
        self.security_monitor.report_connection_attempt(device_id, success, {
