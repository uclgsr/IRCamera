...
class DeviceListWidget(QWidget):
    ...
    def update_devices(self, devices: List[DeviceInfo]):
        ...
        for device in devices:
            ...
            item_text = f"{status_icon} {device.device_name} ({device.device_type})"
    ...
    def _on_refresh_clicked(self):
        ...
        # Signal to parent to refresh device list
        pass
...
class StatusDisplayWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Network metrics
        self._network_status = QLabel("Network: Disconnected")
        self._connected_devices = QLabel("Devices: 0")
        self._data_rate = QLabel("Data Rate: 0 KB/s")
        self._latency = QLabel("Latency: -- ms")
    ...
    def _update_display(self):
        ...
        # This would be connected to actual system monitoring
        # For now, show placeholder values
        import random
        
        self._cpu_progress.setValue(random.randint(10, 60))
        self._memory_progress.setValue(random.randint(20, 80))
        
    def update_network_status(self, connected: bool, device_count: int, data_rate: float, latency: float):
        """Update network status display."""
        status_text = "Connected" if connected else "Disconnected"
        self._network_status.setText(f"Network: {status_text}")
        self._network_status.setStyleSheet(f"color: {'green' if connected else 'red'};")
        
        self._connected_devices.setText(f"Devices: {device_count}")
        self._data_rate.setText(f"Data Rate: {data_rate:.1f} KB/s")
        self._latency.setText(f"Latency: {latency:.1f} ms" if latency >= 0 else "Latency: -- ms")
...
class RealTimePlotWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Create plot widget
        self._plot_widget = pg.PlotWidget(title=self._title)
        self._plot_widget.setLabel('left', 'Value')
        self._plot_widget.setLabel('bottom', 'Time (s)')
        self._plot_widget.showGrid(x=True, y=True)
        
        # Create plot line
        self._plot_line = self._plot_widget.plot([], [], pen='b', width=2)
