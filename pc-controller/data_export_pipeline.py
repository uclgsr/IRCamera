...
class MultiModalDataExporter:
    ...
    def __init__(self, output_dir: str = "data/exports"):
        ...
        self.data_types = {
            ...
            'gsr': {'format': 'timeseries', 'dtype': np.float64, 'columns': ['timestamp', 'gsr_microsiemens']},
            'ppg': {'format': 'timeseries', 'dtype': np.float64, 'columns': ['timestamp', 'ppg_raw']},
            'imu': {'format': 'timeseries', 'dtype': np.float64, 'columns': ['timestamp', 'acc_x', 'acc_y', 'acc_z', 'gyro_x', 'gyro_y', 'gyro_z']},
    ...
    async def export_session(
        ...
        with h5py.File(output_file, 'w') as h5_file:
            ...
            await self._create_synchronized_timeline(h5_file, device_data, clock_offsets)
    ...
    async def _store_device_metadata(
        ...
        for device_id, data in device_data.items():
            ...
            print(f"  📱 Device {device_id}: {len(sensors)} sensors, offset: {clock_offsets.get(device_id, 0)/1e6:.2f}ms")
        ...
        for column in df.columns:
            if column == 'timestamp' or column == 'timestamp_aligned':
                ...
                dataset = sensor_group.create_dataset(
                    column, 
                    data=df[column].astype(np.int64),
                    compression='gzip'
                )
            else:
                # Store numeric data
                dataset = sensor_group.create_dataset(
                    column,
                    data=df[column].astype(np.float64),
                    compression='gzip'
                )
    ...
    async def _store_video_data(
        ...
        if data:
            ...
            if video_files:
                ...
                dataset = sensor_group.create_dataset('video_files', (len(video_files),), dtype=dt)
                dataset[:] = video_files
    ...
    async def _create_synchronized_timeline(
        ...
        for device_id, device_sensors in device_data.items():
            ...
            for sensor_type, sensor_data in device_sensors.items():
                events = self._extract_events(sensor_data, device_id, sensor_type, clock_offset)
        ...
        if all_events:
            ...
            if len(all_events) > 1:
                duration_ns = all_events[-1]['timestamp_aligned'] - all_events[0]['timestamp_aligned']
    ...
    async def _store_analysis_metadata(
        ...
        print(f"  📈 Analysis metadata: {stats['device_count']} devices, {stats['total_sensors']} sensors")
    ...
    device_data = {
        'android_001': {
            'thermal': [
                {
                    ...
                    'matrix': np.random.rand(256, 192).astype(np.float32) * 50 + 20  # 20-70°C
                },
                {
            ...
            'gsr': [
                ...
                {'timestamp': 1700000000020000000, 'gsr_microsiemens': 2.7},  # 20ms later
                {'timestamp': 1700000000040000000, 'gsr_microsiemens': 2.4},  # 40ms later
            ]
        },
        'android_002': {
...
async def validate_exported_file(file_path: str):
    ...
    try:
        with h5py.File(file_path, 'r') as h5_file:
            # Check main groups
            expected_groups = ['session', 'devices', 'sensor_data', 'synchronized_timeline', 'analysis']
