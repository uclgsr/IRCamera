...
class GsrDeviceManager(
    ...
    private fun stopBleScan() {
        ...
        try {
            ...
            mpdc4gsr.core.common.AppLogger
                .w(TAG, "Missing Bluetooth permissions while stopping scan", e)
        } catch (e: Exception) {
            mpdc4gsr.core.common.AppLogger
                .e(TAG, "Unexpected error stopping BLE scan", e)
        } finally {
    ...
    suspend fun initialize(): Boolean =
        withContext(Dispatchers.IO) {
            try {
                ...
                shimmerManager =
                    ShimmerBluetoothManagerAndroid(context, shimmerHandler).also { manager ->
            ...
            coroutineScope.launch {
                while (isActive) {
                    ...
                    val disconnectedDevices =
                        connectedDevices.filter { (address, shimmer) ->
                            try {
    ...
    suspend fun stopDeviceScanning() =
        withContext(Dispatchers.IO) {
            scanJob?.cancel()
            scanJob = null
            stopBleScan()
    ...
    suspend fun disconnectDevice(deviceAddress: String): Boolean =
        withContext(Dispatchers.IO) {
            try {
                val shimmer =
                    connectedDevices[deviceAddress] ?: run {
                        return@withContext false
                    }
    ...
    fun getConnectedDevices(): List<DeviceInfo> =
        connectedDevices.values.map { shimmer ->
            DeviceInfo(
    ...
    fun getConnectedShimmer(deviceAddress: String): Shimmer? = connectedDevices[deviceAddress]
    ...
    private suspend fun startSynchronizedStreamingOnAllDevices(): Boolean =
        try {
            coroutineScope {
                val streamingJobs =
                    connectedDevices.map { (address, shimmer) ->
                        async {
                            try {
    ...
    suspend fun stopMultiDeviceTesting(): Boolean =
        withContext(Dispatchers.IO) {
            try {
                val stopResults =
                    connectedDevices
            ...
            connectedDevices =
                connectedDevices.values.map { shimmer ->
                    ConnectedDeviceInfo(
                        ...
                        connectionState =
                            when (shimmer.bluetoothRadioState) {
    ...
    private fun isValidShimmerDevice(btDevice: BluetoothDevice): Boolean {
        ...
        val hasValidPrefix =
            SHIMMER_MAC_PREFIXES.any { prefix ->
