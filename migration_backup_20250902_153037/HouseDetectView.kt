...
class HouseDetectView : FrameLayout {
    ...
    fun notifyDirInsert(
        ...
        if (dirDetect.isExpand) { // 在展开状态执行复制
            ...
            adapter.notifyItemRangeInserted(
                position + dirDetect.itemList.size + 1,
                dirDetect.itemList.size + 1
            )
        } else {
    ...
    private inner class MyAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
        ...
        override fun getItemViewType(position: Int): Int =
            if (dataList[position] is DirDetect) 0 else 1
            ...
            return if (viewType == 0) {
                HeadViewHolder(
                    LayoutInflater.from(parent.context)
                        .inflate(R.layout.item_report_add_head, parent, false)
                )
            } else {
                ItemViewHolder(
                    LayoutInflater.from(parent.context)
                        .inflate(R.layout.item_report_add_default, parent, false)
                )
            }
        }
        ...
        override fun onBindViewHolder(
            ...
            if (holder is HeadViewHolder) {
                ...
                holder.itemView.tv_image_count_tips.text =
                    SpanBuilder(itemDetect.getImageSize().toString()).appendColor(
                        "/4",
                        0x80ffffff.toInt()
                    )
                ...
                rootView.tv_item_add_text.setOnClickListener { // 添加文字
                    ...
                    if (position != RecyclerView.NO_POSITION) {
                        onTextInputListener?.invoke(
                            Pair(
                                position,
                                dataList[position] as ItemDetect
                            )
                        )
                    }
                }
    ...
    private inner class MyOnLayoutChangeListener : OnLayoutChangeListener {
        override fun onLayoutChange(
            ...
            currentPosition =
                if (dataList[seeFirstPosition] is DirDetect) seeFirstPosition else findDirPosition(
                    seeFirstPosition
                )
