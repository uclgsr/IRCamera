...
class MultiModalSynchronizer:
    ...
    def generate_sync_report(self) -> str:
        ...
        if self.thermal_data is not None and not self.thermal_data.empty:
            ...
            report.append(
                f"Thermal Data: {len(self.thermal_data)} frames over {thermal_duration:.1f}ms")

        if self.gsr_data is not None and not self.gsr_data.empty:
            gsr_duration = self.gsr_data['timestamp_relative_ms'].max() - self.gsr_data[
                'timestamp_relative_ms'].min()
        ...
        if simultaneous:
            ...
            report.append(
                f"Sync quality: {'EXCELLENT' if np.max(time_diffs) < 5 else 'GOOD' if np.max(time_diffs) < 50 else 'ACCEPTABLE' if np.max(time_diffs) < 100 else 'POOR'}")
