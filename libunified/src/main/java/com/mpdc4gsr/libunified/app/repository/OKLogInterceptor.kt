...
class OKLogInterceptor(val isTC007: Boolean) : Interceptor {
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        if (BuildConfig.DEBUG) {.i("--> ${request.method} ${request.url}")
            val requestBody = request.body
            val contentType = requestBody?.contentType()?.toString()
            if (requestBody != null && (contentType == null || contentType == "application/json")) {
                val buffer = Buffer()
                requestBody.writeTo(buffer).v("[ph][ph]：${buffer.readString(StandardCharsets.UTF_8)}")
            }
        }
        ...
        try {
            ...
            if (BuildConfig.DEBUG) {.e("<-- HTTP FAILED: $e")
            }
            throw e
        }

        if (BuildConfig.DEBUG) {                .i("<-- ${response.code}${if (response.message.isEmpty()) "" else ' ' + response.message} ${response.request.url}")
            ...
            if (responseBody != null && (isTC007 || contentType == null || contentType == "application/json")) {
                ...
                if (responseStr.length > 1024) {                        .v("[ph][ph]：${responseStr.substring(0, 1024)} ...[ph][ph][ph][ph][ph][ph][ph]")
                } else {.v("[ph][ph]：$responseStr")
                }
            }
        }
