...
class EnhancedNetworkingExample(private val context: Context) {
    ...
    fun demonstrateEnhancedNetworking() {
        exampleScope.launch {
            try {                certManager.initialize()                webSocketProxy.initializeSecurity(context)                discoveryService.startDiscovery()

                kotlinx.coroutines.delay(5000)

                val discoveredDevices = discoveryService.getDiscoveredDevices()                discoveredDevices.forEach { device ->"
                    )
                }
                ...
                if (discoveredDevices.isNotEmpty()) {
                    ...
                    if (pcController != null) {                        val syncResult = timeSyncService.synchronizeTime(
                            pcController.ipAddress,
                            pcController.port
                        )
                        if (syncResult.isSuccess) {                            val syncTimestamp =
                                timeSyncService.getSynchronizedTime(syncResult.clockOffsetMs)                        } else {                        }
    ...
    private fun demonstrateSecureWebSocket(deviceName: String = "TS004_DEMO_DEVICE") {        webSocketProxy.startWebSocket(deviceName)
